    ...
    public static string Write(List<KvLine> lines, bool makeCompatible = true)
    {
        ...
        foreach (var line in lines)
        {
            ...
            if (line.LineType == KvLineType.Comment)
                result = result.Insert(result.Length, $"#{line.Key}");
            else if (line.LineType == KvLineType.KeyValue)
                result = result.Insert(result.Length, $"{line.Key}={formattedValue}");
...
/// </summary>
public struct KvLine(KvLineType lineType, string key = "", string value = "")
{
    public KvLineType LineType = lineType;
